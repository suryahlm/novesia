/**
 * VPS Scraper - Scraping Only (Fixed with ON CONFLICT)
 * 10 novels, all chapters, no translation
 */
...
const CONFIG = { MAX_NOVELS: 10, DELAY_MS: 2000, TIMEOUT_MS: 60000 };
...
async function saveNovel(novel) {
    ...
    try {
        ...
        const id = cuid();
        await client.query(`INSERT INTO "Novel" (id,title,slug,synopsis,cover,author,status,"sourceUrl","isManual","totalViews","avgRating","ratingCount","isPremium","coinCost","freeChapterLimit","createdAt","updatedAt") VALUES ($1,$2,$3,$4,$5,$6,$7,$8,false,0,0,0,false,5,0,NOW(),NOW()) ON CONFLICT DO NOTHING`, [id, novel.title, slug, novel.synopsis, novel.cover, novel.author, novel.status, novel.url]);
...
async function saveChapter(novelId, chapter, content) {
    ...
    try {
        ...
        await client.query(`INSERT INTO "Chapter" (id,"novelId","chapterNumber",title,"contentOriginal","contentTranslated","wordCount","isPremium","coinCost",views,"sourceUrl","createdAt","updatedAt") VALUES ($1,$2,$3,$4,$5,'',$6,false,0,0,$7,NOW(),NOW()) ON CONFLICT ("novelId", "chapterNumber") DO NOTHING`, [id, novelId, chapter.number, chapter.title, content, wordCount, chapter.url]);
        return true;
    } catch (err) {
...
async function main() {
    log('=== VPS SCRAPER - 10 NOVELS, ALL CHAPTERS ===');
    ...
    try {
        ...
        for (let i = 0; i < novels.length; i++) {
            ...
            log(result.isNew ? 'New novel' : 'Exists');
            await sleep(CONFIG.DELAY_MS);
            const chapters = await scrapeChapterList(page, novel.url);
            let saved = 0;
            for (const ch of chapters) {
                const client = await pool.connect();
                const exists = await client.query('SELECT id FROM "Chapter" WHERE "novelId"=$1 AND "chapterNumber"=$2', [result.id, ch.number]);
                client.release();
                if (exists.rows.length > 0) continue;
                await sleep(CONFIG.DELAY_MS);
                log(`  Ch.${ch.number}/${chapters.length}...`);
                const content = await scrapeChapterContent(page, ch.url);
                if (content && content.length > 100) {
                    await saveChapter(result.id, ch, content);
                    total++; saved++;
                }
            }
            log(`  Done: ${saved} new chapters`);
        }
        log(`\n=== COMPLETE! ${total} chapters saved ===`);
